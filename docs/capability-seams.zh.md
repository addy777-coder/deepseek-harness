...
flowchart LR
  ...
  svc_usageController["ctx.usageController<br/>Recorded session usage controller"]
...
| --- | --- | --- | --- | --- | --- | --- |
| `ctx.network` | `seam` | [`network`](../packages/network/network) | [`network-openvpn`](../packages/network/network-openvpn) | [`llm-pi-ai`](../packages/llm/llm-pi-ai), [`api-vpn-controller`](../packages/api/vpn-controller) | - | 已配置的模型目标通过应用自有的用户态 VPN 访问，不改变系统网络；隧道不可用时拒绝请求。 |
| `ctx.vpnController` | `core` | [`api-vpn-controller`](../packages/api/vpn-controller) | - | [`client-ui-vpn`](../packages/client/ui-vpn) | - | 通过应用 Remote API 暴露脱敏 VPN 状态和经验证的连接控制。 |
| `ctx.usageController` | `core` | [`api-usage-controller`](../packages/api/usage-controller) | - | [`client-ui-usage`](../packages/client/ui-usage) | - | 基于优先使用运行中会话的观察，聚合已记录 token 用量与用户活动，无需启动智能体。 |
