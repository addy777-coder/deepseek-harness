...
| `ctx.vpnController` | `core` | [`api-vpn-controller`](../packages/api/vpn-controller) | - | [`client-ui-vpn`](../packages/client/ui-vpn) | - | Exposes redacted VPN state and validated connection controls through the application Remote API. |
| `ctx.usageController` | `core` | [`api-usage-controller`](../packages/api/usage-controller) | - | [`client-ui-usage`](../packages/client/ui-usage) | - | Aggregates recorded token usage and user activity over live-preferred session observations without starting agents. |
