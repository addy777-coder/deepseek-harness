...
手动输入的模型在你声明之前一律按纯文本对待，因为没有任何环节能去询问端点接受哪些模态。请在**模型目录**中展开该模型，开启**支持图片输入**并保存。随后，该行与模型选择器都会显示**视觉**标签。

该开关会为 pi-ai 模型写入 `input: [text, image]`，为 DeepSeek 直连模型写入 `inputModalities: [text, image]`。等效的 pi-ai 设置如下：
...
**设置 → 模型**顶部的**图片识别**卡片允许一个已选视觉模型为纯文本会话模型提供图片信息。请先为视觉模型开启**支持图片输入**，再在**图片识别模型**下选择并保存。该设置同时保存提供方和模型，因此不同提供方上的同名模型仍能区分。

纯文本模型遇到用户上传、`read_image` 结果、MCP 结果、ACP 提示词或 Subagent 后续消息时，已选视觉模型只会收到来源消息的文本和有序图片。其事实报告和精确路由会在文本请求前写入会话。普通 Chat 视图隐藏该内部上下文；Trajectory 与会话导出会保留它，以便诊断和回放。

每条来源消息只识别一次。在新消息中重新附加相同图片时，会结合该消息的文本再次识别。识别失败、被取消、返回空内容、调用工具或被截断时，文本请求会停止且不会保存部分报告；原始图片仍可供下次重试。

不配置**图片识别模型**时，纯文本图片准入保持关闭。应用不会自动选择提供方，也不会在保存时发送产生费用的测试图片。即使会话模型使用另一个提供方，图片仍会发送到所选识别模型的提供方。
...
- **图片在发送前被拒绝**：会话模型未声明图片模态，并且没有配置有效的图片识别模型。请为视觉模型开启**支持图片输入**并在**图片识别模型**中选择它，或把会话切换到视觉模型。
- **已配置的图片识别模型不可用**：它的提供方已删除、模型已消失或图片能力已关闭。请返回**设置 → 模型**，选择其他模型或清除此设置。
