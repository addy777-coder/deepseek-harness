...
| `dsh --profile desktop` | Run the Desktop Host inside the Electron-owned Utility Process. |
...
The invoking directory is the default workspace root. The `web`, `desktop`, `headless`, `sdk`, `sdk-minimal`, and `acp` profiles auto-initialize on first use from shipped templates; any other profile must be created through `dsh plugin`. DSH Desktop supplies the required Utility Process channel for `desktop`; an ordinary terminal invocation fails instead of opening a server.
