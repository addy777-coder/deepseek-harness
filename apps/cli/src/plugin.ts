...
import { reconcileProfileBundles } from '@deepseek-ai/dsh-app-boot/profile-plugins'
...
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  try {
    directory = resolveBundleDir(NAME, packageName, INSTALL_ANCHOR, profileDir)
  } catch {
    return false
  }
  return readProfileManifest(NAME, directory).dsh?.bundle?.patch !== undefined
}

function reconcilePlugins(before: ProfileManifest, profileDir: string) {
  const after = readProfileManifest(NAME, profileDir)
  const bundleDependencies = new Set(Object.keys(after.dependencies ?? {})
    .filter(packageName => exportsPatch(packageName, profileDir)))
...
export function runPlugin(profile: string, args: readonly string[]): number {
  ...
  if (exitCode === 0) {
    const reconciliation = reconcilePlugins(before, dir)
