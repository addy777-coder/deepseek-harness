...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  'packages/api/usage-controller': { kind: 'none', reason: 'Read-only Session accounting exposes statistics to the UI without starting Agents or changing model requests.' },
  'packages/api/vpn-controller': { kind: 'none', reason: 'Local VPN configuration and redacted status do not add model messages, tools, or Session events.' },
  'packages/client/ui-usage': { kind: 'none', reason: 'The settings page renders recorded history statistics and never constructs model input.' },
  'packages/client/ui-vpn': { kind: 'none', reason: 'The settings page controls private network state without constructing model input.' },
  'packages/network/network': { kind: 'none', reason: 'The service declares HTTP destination and tunnel ownership; model consumers own request content.' },
  'packages/network/network-openvpn': { kind: 'none', reason: 'The tunnel carries existing HTTP requests without adding model messages or changing their contents.' },
