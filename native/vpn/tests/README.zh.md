...
运行 `pwsh -File native/vpn/tests/dependency-install.test.ps1`，通过本地子进程 fixture 检查有界的依赖下载重试。它验证恢复和拒绝行为，不发起网络请求，也不需要已编译的 VPN 二进制。`pwsh -File native/vpn/tests/toolchain.test.ps1` 检查多个同名工具的 PATH 优先顺序，并拒绝缺失的可执行文件。
