...
提供方可以显式选择 `network: vpn`，并指定 Anthropic Messages 端点和 `apiKeyEnv` 引用。[网络提供方](../../network/network-openvpn/README.zh.md)拥有隧道与已配置目标。模型请求和发现使用私有 HTTP 分发，不回退到直连；VPN 发现要求使用已保存的端点和协议。省略 `network` 时保留 SDK 的直连行为。
