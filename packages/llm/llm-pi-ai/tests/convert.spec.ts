...
describe('toStreamChunks', () => {
  ...
  it('maps text events to text blocks', async () => {
    ...
    expect(chunks).toEqual([
      ...
      { type: 'usage', usage: {
        inputTokens: 3, outputTokens: 2, totalTokens: 5, cacheReadTokens: 0, cacheWriteTokens: 0,
      } },
      {
        type: 'finish',
        reason: { kind: 'stop' },
  ...
  it('maps error events to error finish chunks (in-stream error style)', async () => {
    ...
    expect(chunks).toEqual([
      { type: 'usage', usage: {
        inputTokens: 1, outputTokens: 0, totalTokens: 1, cacheReadTokens: 0, cacheWriteTokens: 0,
      } },
...
describe('mapStopReason / mapUsage', () => {
  ...
  it('maps cache buckets always, zeros included', () => {
    ...
    // pi-ai reports zeros rather than absence; an explicit zero keeps turn and
    // session folds whole so a later cache hit is not dropped.
