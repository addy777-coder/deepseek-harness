...
A provider may explicitly select `network: vpn` with an Anthropic Messages endpoint and an `apiKeyEnv` reference. The [network provider](../../network/network-openvpn/README.md) owns its tunnel and configured destinations. Model requests and discovery use private HTTP dispatch without direct fallback; VPN discovery requires the saved endpoint and protocol. Omitted `network` retains direct SDK behavior.
