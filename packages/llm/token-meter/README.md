...
`normalizeTokenUsage(usage)` exposes the same per-attempt normalization through `./client` for read-only history accounting. It returns no result for incomplete, inconsistent, negative, or unsafe counts, and preserves absent cache buckets.
