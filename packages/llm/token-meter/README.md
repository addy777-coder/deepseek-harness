...
The browser-safe `@deepseek-ai/dsh-token-meter/client` entry exports `normalizeTokenUsage(usage, route?)` and its `NormalizedTokenUsage` result type. This pure helper accepts a consistent provider-reported total or derives one only when the disjoint input, cache-read, cache-write, and output buckets are complete. It preserves optional bucket and route facts, treats reasoning as an output subdivision, and returns `undefined` for incomplete, contradictory, or unsafe counts; it never estimates missing usage. Both the Turn fold and the [global usage controller](../../api/usage-controller/README.md) use this validation.

`deriveTurnTokenUsage(events)` folds one complete Turn into exact per-attempt and total usage for browser consumers. Missing lifecycle evidence, unsafe counts, or contradictory exact totals return no result. Cache aggregates always appear, with omitted attempt buckets counted as zero; reasoning and route aggregates appear only when every contributing attempt reports them.
