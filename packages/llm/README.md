...
The llm group provides the harness's model-call capability: one provider-neutral service through which any composition streams requests to a model provider, plus adapters, auxiliary image recognition, provider-specific request metadata, retry execution, and measurement. The core `llm` package defines the message, content-block, stream-chunk, and image-recognition service vocabulary every plugin and the session log use; provider adapters translate a provider's wire format into that vocabulary; `image-recognition-llm` supplies durable visual reports to text-only routes; `llm-retry` re-runs failed requests at durable agent-step boundaries; and `token-meter` measures request and context pressure from the durable log. This page maps the group; each package README owns its per-package contract.
...
| [`image-recognition-llm/`](image-recognition-llm/README.md) | Uses one selected image-capable route to add durable visual reports before text-only model requests | `ctx.imageRecognition` |
