---
description: "DSH GUI 的主题与正文字号设置：--dsw-* token 样式表、ThemeRuntime 状态、「通用」设置行与插件前引导。"
...
`dsh-client-ui-theme` 让 Web 与 Desktop GUI 用户在设置中选择 `light`、`dark` 或 `system`，并把会话正文字号设为 12 至 17 px。回环客户端把两个值存入 `ui-theme` 设置命名空间，本地提供方默认将其持久化到 `$DSH_HOME/settings.yaml`。插件通过 `prefers-color-scheme` 解析 `system` 并发布不可变的 `ThemeSnapshot`；ui-layout 把每份快照应用到 document。本包还提供 `--dsw-*` token 样式表，并贡献同步引导，使所选调色板与字号在外壳加载前生效。第三方主题可通过 `ctx.theme` 注册别名 token 覆盖。
...
用户从设置（「通用」分区）的两行中切换配色方案与正文字号；在本地 GUI 载体（回环 Web 或 Desktop）上，两个选择都会跨重启持久化。功能插件通过 `ctx.theme` 消费当前快照，并在 CSS 中读取 `--dsw-*` token；它们不自行管理主题状态。

`--dsw-alias-chart-1` 至 `--dsw-alias-chart-5` 为有序数据系列提供颜色，`--dsw-alias-chart-other` 表示合并后的其余系列。每个别名都有浅色和深色值；[使用统计页](../ui-usage/README.zh.md)在每日柱状图、模型环图与图例之间共用这些颜色。
...
Host 侧通过 `ClientBootRegistry` 贡献已注册的 `ui-theme` 设置或 schema 默认值。Web 把该条目渲染进每份 index 响应；Desktop 则通过 IPC boot payload 携带它。Renderer 在加载页面渲染前设置 `color-scheme`、`body[data-ds-dark-theme]` 与 `--dsh-content-font-size`，因此首帧绘制就采用所选调色板与字号。
...
在本地 GUI 载体上，服务先以 schema 默认值立即提供自身，随后加载 `ui-theme` 命名空间，并把每次通过的主题或字号变更经 Host settings API 写入。收到推送的设置变更时或重连后都会重新拉取该命名空间。非 loopback Web 页面不会创建该 Host-backed scope。该持久化边界由 [Host 支撑的偏好笔记](../../../.agents/notes/implemented/bug-fix/2026-08-06-host-backed-web-preferences.zh.md) 拥有。
