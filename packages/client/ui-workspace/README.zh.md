...
**新建分区**会创建命名且可折叠的侧栏分区。分区支持重命名、删除和排序；项目行位于独立 Session 行之前，两类各自保持手动拖动顺序。行菜单提供分区归类、恢复默认位置和上下移动操作。分组视图只在有效落点提交移动；Escape 或在目标外松手会取消拖动。保存失败时保留已确认的布局，并提供重试。

将 Session 单独移入分区会隐藏其原 Workspace 下的行，同时保留工作目录和 Workspace 记账。恢复默认位置会使它回到所属 Workspace 或“未分组”。删除分区会按区内顺序把项目追加到默认区域，并恢复独立 Session 的默认位置。Host 持久化并广播名称、归类和顺序；折叠状态保存在浏览器本地。单列表和搜索独立于分区，每个 Session 只显示一次。
