...
describe('Presented workspace file native open route', () => {
  ...
  it('refuses final file symlinks when the Host can create them', async ({ skip }) => {
    ...
    try {
      ...
      if (process.platform === 'win32' && error instanceof Error && 'code' in error && error.code === 'EPERM') {
        skip('Windows file-symlink creation requires Developer Mode or the symbolic-link privilege')
      }
      throw error
    }
    expect((await open()).status).toBe(404)
