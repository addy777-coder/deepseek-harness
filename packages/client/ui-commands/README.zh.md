...
宿主命令通过命令目录发布其菜单描述，因此 `/` 菜单经由 `command` 命名空间的 `description.<name>` 键渲染：活动 locale 提供该键时以键值替换 wire 原文，未收录的命令名按原文回退（绝不显示字典 key），且每一轮候选构建都重新读取活动 locale——切换语言后无需任何重新注册即可刷新菜单。客户端贡献仍然由其所属 feature 的字典自行提供描述。
...
- **客户端贡献描述是注册表持有文本**——贡献行（如 `/model`）在注册时经其所属 feature 的字典捕获描述，并在重新注册前保持该语言；宿主行则于每轮候选构建重新读取活动 locale。
