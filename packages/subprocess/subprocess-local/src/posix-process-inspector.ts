/** POSIX process inspection without native addon loading, shared by terminals and supervisors. */
...
// This macOS format omits the session id; stat distinguishes running members
// from zombies awaiting their parent's waitpid.
function macProcessTable(internals: ProcessInspectorInternals): ProcessRow[] {
  return internals.exec('/bin/ps', ['-axo', 'pid=,ppid=,stat=,lstart=']).split('\n').flatMap((line) => {
...
class MacProcessInspector extends PosixProcessInspector {
  ...
  isAlive(identity: ProcessIdentity): boolean {
    return macProcessTable(this.internals)
      .some(entry => entry.pid === identity.pid && entry.started === identity.started && !quiescent(entry.state))
  }
...
/**
 * Create a Linux or macOS inspector without loading native addons.
 * @param platform - target Node platform; rejects platforms other than Linux and macOS.
 * @param arch - target CPU architecture for Linux syscall numbers.
 * @param internals - filesystem and process operations, injectable for deterministic tests.
