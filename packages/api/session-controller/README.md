...
`session/modelCatalog` carries each resolved model's optional input modalities so Clients can label and filter explicit image capability. Prompt admission sends an image directly when the selected model accepts it; an explicitly text-only model requires a verified `ctx.imageRecognition` target before the attachment batch is persisted and queued.
