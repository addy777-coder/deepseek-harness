...
Token totals include recorded conversation, subagent, and compaction usage. The [token-meter normalizer](../../llm/token-meter/README.md) accepts a consistent provider total, or derives a total only from complete disjoint input, cache-read, cache-write, and output buckets. Reasoning is an output subdivision and is not added again. Each durable Assistant message or attempt contributes one sample: final-message usage takes precedence over the last embedded stream sample. Invalid or incomplete final counts remain missing rather than falling back to an earlier sample. Retries count separately, and settled failures or cancellations retain their reported usage. Tokens belong to the sample’s local date and recorded provider/model pair; unsettled streams contribute no token total.
