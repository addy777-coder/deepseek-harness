...
token 总量包含已记录的对话、子智能体和压缩用量。[token-meter 归一化函数](../../llm/token-meter/README.zh.md)接受一致的提供方总量，或仅在互不重叠的输入、缓存读取、缓存写入和输出分桶完整时计算总量。推理属于输出细分，不会再次相加。每次尝试保留最后一个有效采样；最终消息用量替换同次尝试的流式用量，重试分别计数，已报告用量在失败或取消后仍然保留。token 按采样的本地日期及记录的提供方/模型组合归属。
