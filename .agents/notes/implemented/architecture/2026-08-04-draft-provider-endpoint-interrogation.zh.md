...
- `LlmModelDiscoveryRequest` 携带可选的草稿字段：`provider`、`baseURL`、`api`、`apiKey` 和 `network`。操作还接受取消 signal。发现要求 `provider` 与 `baseURL` 至少有一个。`provider` 之所以存在，是因为适配器已经描述过的路由直接由它自己的注册表作答、完全不联网；只有它未描述的路由才会抵达某个端点。这条路径不写 settings 与 credentials。已配置且具名的路由会在 Host 内读取已存凭据和部署方持有的 profile `headers`：凭据只写，而精选的 Models 页面不编辑 headers，因此页面草稿无法重建两者。键入的密钥优先于已存凭据，profile headers 则仍随请求发送。
- `LlmDiscoveredModel` 除 `id` 外每个字段都可选，因为大多数列表只公布 id。回复是候选而非 catalog：采纳其中一条的界面仍要补上适配器所需的容量。
- `llm.discoverModels` 把同一份草稿送过协议层。它的 `apiKey` 是一次性承载机密的载荷，绝不被存储或回显。它确实会像其他承载机密的载荷一样随客户端外发信封同行，`subscribeEnvelopes()` 观察者看得到；把那个抽头脱敏是整个配置面的改动，不该由这一个方法独自决定。Connection 用与完整 Host API 相同的会话认证该方法：它让宿主向调用方选定的 URL 发起 GET 并回报结果，匿名调用者绝不能获得这类探测能力。每一种拒绝都折叠为 `model-discovery-failed`，其消息是适配器自己的文本，details 点名被询问的端点，绝不点名所提供的凭据。

`dsh-llm-pi-ai` 通过带 bearer 认证的 `GET {baseURL}/models` 读取 `openai-completions` 与 `openai-responses`。Anthropic Messages 使用 `GET {baseURL}/v1/models`，并带有 `x-api-key` 与 `anthropic-version` 标头。Profile 解析会拒绝 Fetch 无法表示的名称与值，因此格式错误的部署 header 会在询问前以配置错误报告。已配置的 profile headers 最先装入；JSON accept header、协议拥有的标头、键入或已存的凭据，以及 Harness attribution 随后依次以大小写不敏感方式赢得冲突。Azure 虽然源自 OpenAI，但因使用 `api-key` 标头并要求 `api-version` 查询参数而被排除；Codex 则使用 OAuth。将两者当作普通列表请求会把认证错误误报为没有模型。其余协议一律以 `DISCOVERY_UNSUPPORTED` 回答，让界面回退到手工填写，而不是把猜错的响应形状报成一个空提供方。`baseURL` 按前缀而非待解析 URL 处理，因此 `https://gateway.example/openai/v1` 这类部署路径会保留其路径段。回复在四兆字节上限下读取，且上限落在实际收到的字节上——端点是用户自己填的 URL，因此会先看声明的 `content-length` 作为善意提示，但绝不把它当作边界；这与 `dsh-web-fetch` 面对自己的调用方提供 URL 时所用的两段式形状一致。

VPN 发现使用已保存提供方的私有 fetch；草稿的网络、端点或协议与保存配置不一致时，请求会被拒绝。它绝不会静默地通过直连网络探测已保存的 VPN 目标。[应用自有 VPN 决策](2026-09-09-application-owned-vpn.zh.md)负责定义这项针对直连草稿发现流程的例外。
...
**让 Host 读取整个已存 profile，而不是接受草稿。** 对已配置好的提供方来说，不会有机密跨越协议层。但这样一来新增提供方就必须先保存一份不可用的配置，而端点已改却尚未保存的表单会静默地去询问旧地址。直连草稿仍是端点和协议的权威来源。Host 拥有的输入是只写的已存凭据与部署 profile headers。VPN 草稿必须匹配已保存目标，因为网络提供方拥有允许列表。

**询问 pi-ai 的每一种协议。** 共用列表信封并不能证明协议兼容：认证标头、URL 参数与分页可能不同。猜测这些字段可能让不支持的协议与「该提供方没有模型」无法区分。一个明说自己无法被询问的协议，会把用户送去手工填写——那正是既定的回退路径。
...
发现通过已认证的配置 API 传递一次性凭据。覆盖范围取决于明确支持的协议；其他协议需要手动输入。VPN 目标必须先保存再发现。因为没有任何环节重新发起询问，模型列表只反映最近一次编辑时的内容。
