...
The application owns connection, reconnect, cancellation, and process-tree cleanup. A private stdin pipe delivers bootstrap credentials and ties the helper to the Host’s lifetime. Explicit disconnect cancels reconnect intent; authentication failures stop retries. The [Web bundle](../../../../packages/bundle/web-app/README.md), also consumed by Desktop, selects the provider, controller, and settings UI. Deployments supply the native helper path and checksum through the provider configuration.

The [native distribution](../../../../native/vpn/README.md) contains the GPL-3.0-only helper, license notices, and corresponding-source ZIP for the linked executable. Deployments distribute these assets together; the official Desktop installer does not bundle this helper. A subprocess keeps the protocol and lifetime explicit; it does not remove the helper's source-distribution obligations.
