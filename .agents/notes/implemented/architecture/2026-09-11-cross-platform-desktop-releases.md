...
Build-tool discovery selects one executable in PATH order. Runtime verification resolves the packaged directory to its real filesystem path before enforcing dependency containment. OS aliases such as macOS `/var` → `/private/var` therefore retain the same restriction against external modules.
