...
**Models 配置界面会显式编辑 `input`。** [文本模型图片识别决策](../feature/2026-09-04-text-model-image-recognition.zh.md)取代了本说明早先的展示选择，同时保留上述模态优先级。模型列表编辑器写入 `[text]` 或 `[text, image]`，保留不由它编辑的字段，并把接口发现的模型默认为文本，因为模型列表接口不披露模态。
...
自定义提供方下的视觉模型通过 Models 页面或设置文档声明 `input: [text, image]`；当路由列出的所有未描述模型都接受图片时，也可以在路由上声明一次。直接图片准入和 `read_image` 随即使用该能力；已配置的图片识别路由还可以为明确仅支持文本的会话模型处理图片。什么都不写的部署保留文本回退，不会代替模型声明图片能力。
...
Models UI 测试固定显式文本/图片写入、不相关模型字段的保留以及视觉标签。Pi-ai 发现测试固定已安装目录条目保留已知模态，而通用接口列表继续保持未知。准入点仍由 [`read-image`](../../../../snapshots/session/read-image/) 与 [`read-image-text-route`](../../../../snapshots/session/read-image-text-route/)覆盖；图片识别功能负责单独的文本路由集成覆盖。
