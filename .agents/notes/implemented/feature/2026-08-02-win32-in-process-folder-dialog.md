...
The result decoder uses `koffi.decode.string16()` to copy the NUL-terminated string from the `IShellItem::GetDisplayName` allocation before `CoTaskMemFree`. External ArrayBuffer views are unavailable in Electron and can terminate its Node helper with exit code 134. The copying decoder also avoids a fixed-size scan beyond the allocation and preserves UTF-16 code units, including surrogate pairs.
...
- Dialog appearance remains a manual Windows check. The source smoke proves open/abort/unwind, and the [Desktop native-picker helper](../../../../apps/desktop/tests/native-directory-picker.ts) selects a temporary Unicode directory and cancels a second dialog through the real built worker.
...
- The Desktop Electron scenario runs the native-picker helper with the selected Electron or packaged executable and its installed worker, asserting both returned paths and clean process exits.
