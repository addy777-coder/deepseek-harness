...
原生依赖缓存仅保留下载归档及以 ABI 为键的 vcpkg 库缓存，并按目标和依赖输入区分。源码检出、已安装目录与应用包仍重新构建并验证。安装器仅对已确认的临时传输失败重试；编译、永久 HTTP 失败及哈希不匹配仍会报错。Windows 目录选择器测试将所创建目录解析为真实路径，因为 runner 的 TEMP 可能含有 8.3 别名，而原生选择器返回长路径。

构建工具发现按 PATH 顺序选择单个可执行文件。运行时校验在检查依赖是否位于包内之前，先将打包目录解析为真实文件系统路径。因此，macOS `/var` → `/private/var` 等操作系统别名仍保留相同的外部模块限制。
